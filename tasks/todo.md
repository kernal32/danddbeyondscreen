...
- **2026-04-02:** **`ddb-campaign-initiative-bar.user.js` v1.6.9:** **Settings panel** (⚙ button in header): colour theme swatches (Crimson / Arcane / Forest / Parchment) + card density toggle (Comfortable / Compact); settings persist to `localStorage` under `ddbInitBarSettingsV1`; `applyBarSettings` writes CSS custom properties live to `.dib-wrap` so no rebuild needed. **Theme system**: card palette (`--dib-pc-gold/teal/panel/bg-*`) wired via `--dib-pc-*` bridge variables; Parchment flips `--dib-text` to dark. **Density**: `--dib-card-padding` / `--dib-avatar-size` / `--dib-stat-val-size` drive avatar, card padding, and stat-badge + passive numbers. **Passive font fix**: `.dib-pc-pass-num` now `var(--dib-stat-val-size)` matching stat-badge. `node --check` OK; public copy synced.
- **2026-04-02:** **`ddb-campaign-initiative-bar.user.js` v1.6.8:** `deathSavesFromCharacter` always fills each side independently — `hpi.deathSaveInfo` consulted whenever either `s` or `f` is missing (fixes failures=0 when successes came from flat `hitPointInfo` but fails lived in nested `deathSaveInfo`); `c.deathSaves` / root object checked per side not as a block; `__pickDsScalar` normalises numbers, arrays, and numeric strings. `pickDeathSaveCountsFromObject`: `deathSaveFailures`, `failCount`, `successRolls`/`failRolls` aliases added. **Layout:** death-save pips and passives now share one row (`.dib-pc-ds-pass-row`): pips left-aligned (success+fail grouped flush left); Perc./Inv./Ins. compact on the right with 16px numbers. Public copy synced; `node --check` OK.
